...
trait AuthorHandlers {
  ...
  // Handler for INSERT /api/web/author?name=paul&surname=sasieta&nationality=spain
  @throws(classOf[ApiAuthorException])
  def insertAuthor(author: Author): Unit

  // Handler for PATCH /api/web/author?name=paul&surname=sasieta&name=pepe&surname=seseta
  @throws(classOf[ApiAuthorException])
